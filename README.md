...
🛑 **Before you run `apt upgrade` / `apt full-upgrade`:** Bobcat 300 hardware is not yet mainlined in Armbian, and a free kernel/U-Boot upgrade can break your boot. Jump to **[Upgrade Safety](#upgrade-safety--holding-kernelu-boot-packages)** and hold those packages first — do this right after your first successful boot, before you forget.
...
The **G280 / G285 boots and runs entirely from the SD card.**  
...
- Create a new user  
🛑 **Next step:** before you run any `apt upgrade`, go hold the kernel/U-Boot packages — see **[Upgrade Safety](#upgrade-safety--holding-kernelu-boot-packages)** below.
...
## Notes
- G280 / G285 → Runs fully from SD card  
